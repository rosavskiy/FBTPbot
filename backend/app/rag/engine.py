...
ГЛАВНОЕ ОГРАНИЧЕНИЕ: Ответ ДОЛЖЕН быть НЕ БОЛЕЕ 900 символов (кириллица). Пиши КРАТКО, только суть и действия. Без вступлений, без "рад помочь", без повторения вопроса.
...
# Лимит ответа для интеграции с Фармбазис (2048 байт UTF-8 ≈ 900 символов кириллицы)
...
class RAGEngine:
    ...
    async def ask(
        ...
        # 5.1. Обрезаем ответ до лимита 2048 байт для интеграции с Фармбазис
